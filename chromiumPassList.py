...
def send_telegram_message(bot_token, chat_id, message):
    ...
    max_message_length = 4000  # Batasan panjang pesan untuk Telegram

    # Memeriksa panjang pesan, jika lebih panjang dari batasan
    if len(message) > max_message_length:
        # Memecah pesan menjadi beberapa bagian
        message_parts = [
            message[i : i + max_message_length]
            for i in range(0, len(message), max_message_length)
        ]
        # Mengirim setiap bagian pesan ke bot Telegram
        for part in message_parts:
            bot.sendMessage(chat_id, part)
    else:
        # Jika pesan cukup pendek, kirim pesan langsung
...
if __name__ == "__main__":
    ...
    chat_id = ""  # Gantilah dengan ID obrolan Telegram Anda
    bot_token = ""  # Gantilah dengan token bot Telegram Anda
